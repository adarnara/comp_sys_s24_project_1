...
void
close_pipes_except(int *fd, int fd_size, int except_index);
...
//    printf("Terminating process ID %d\n", pid);
    printf("Preparing to kill process ID: %d with signal: %d.\n", pid, signal);
    kill(pid, signal);
    ...
    printf("Exiting kill_c_proc, PID: %d\n", pid); // Added line
}
    ...
    for (int pos = 0; pos < depth_with_log2; pos++) {
        ...
        for (int i = 0; i < user_input_num_of_max_children; i++) {
            if (child_gen_process_pid == getpid() && child_proc_arg < PN) {
                ...
                printf("Attempting to fork a process, current child process arg: %d.\n", child_proc_arg);
                pid = fork();
                if (i != 0) {
...
//            send_raise_with_signal_custom_handler(SLEEP_TIME);
    ...
    printf("Program is exiting normally.\n"); // Added just before the successful exit
    return STATUS_SUCC;
}
...
rule_1_2_3(int child_arg, int max_a, int max_b, pid_t t_pid, int str)
{
    printf("Entering rule_1_2_3: Child Arg: %d, Max A: %d, Max B: %d, Target PID: %d, STR: %d\n", child_arg, max_a, max_b, t_pid, str); // Added print

    // rule 1
    if (max_b > max_a) {
        printf("Rule 1 triggered. Sending SIGCONT to PID: %d.\n", t_pid);
        kill(t_pid, SIGCONT);
        sleep(SLEEP_WITH_100);
        printf("Exiting after Rule 1 with child_arg: %d\n", child_arg); // Added print
        exit(child_arg);
    }
        // rule 2
    else if (H <= (str+1) / PIPE_DATA_ELEMENTS) {
        printf("Rule 2 triggered. Preparing to kill PID: %d with SIGKILL.\n", t_pid); // Added print
        kill_c_proc(t_pid, SIGKILL);
        printf("Exited Rule 2\n"); // Added print
    }
        // rule 3
    else {
        printf("Rule 3 triggered. Sending SIGCONT followed by SIGINT and SIGQUIT to PID: %d.\n", t_pid);
        kill(t_pid, SIGCONT);
        sleep(SLEEP_TIME);
        kill(t_pid, SIGINT);
        sleep(SLEEP_TIME);
        kill(t_pid, SIGQUIT);
        printf("Exited Rule 3\n"); // Added print
    }

    printf("Exiting rule_1_2_3\n"); // Added print
}
